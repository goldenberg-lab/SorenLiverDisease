...
# To do list: 
# 1. Import Toronto/McGill dataset 
# 2. Import imputer 
# 3. Import all algorithms 
# 4. For each patient, go through algorithms and make prediction 
# 5. Store predictions in a CSV file 
# 6. Store patient information in the CSV file as well (e.g. blood test results)

# Step 1. Import the data 
sql = "SELECT * FROM _TorontoHoldOut30"
cnxn = db.connect((r'DRIVER={Microsoft Access Driver (*.mdb, *.accdb)};'r'DBQ=C:\Users\Soren\Desktop\Thesis\Fibrosis.accdb;'))
...
variables = ['Albumin', 'ALP', 'ALT', 'AST', 'Bilirubin', 'Creatinine', 'INR', 'BMI', 'Platelets', 'Hb', 'WBC']
...
# Note, do not change the features used, since these were what the algorithms were trained on
...
data = data[['dataset', 'reckey_enc', 'bx_date', 'Fibrosis', 'SVM_prob', 'RFC_prob',
       ...
       'Platelets', 'BMI', 'Diabetes', 'Hb', 'WBC', 'missing_Albumin', 'missing_ALP', 'missing_ALT', 'missing_AST',
       'missing_Bilirubin', 'missing_Creatinine', 'missing_INR', 'missing_BMI',
       'missing_Platelets', 'missing_Hb', 'missing_WBC', 'Etiology']]

data.to_csv('final_analysis_results/TLC30_results.csv')
