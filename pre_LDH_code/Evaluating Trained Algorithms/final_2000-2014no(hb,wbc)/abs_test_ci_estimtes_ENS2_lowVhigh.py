...
elif (ds == 'McGill_noNAFL'):
    sql = "SELECT * FROM _McGillData Where Neoplasm=0 AND missingness <= 3 AND (Fibrosis=0 OR Fibrosis=1 or Fibrosis=4) AND NAFL=0"
elif (ds == 'McGill_NAFL'):
    sql = "SELECT * FROM _McGillData Where Neoplasm=0 AND missingness <= 3 AND (Fibrosis=0 OR Fibrosis=1 or Fibrosis=4) AND NAFL=1"
elif (ds == 'Expert'):
...
ENS3_low_results = np.empty([8,n_iterations]) 
ENS3_high_results = np.empty([8,n_iterations]) 
...
#    FIB4_results[:,i] = 100*np.transpose(np.array([FIB4_sens, FIB4_spec, FIB4_ppv, FIB4_npv, FIB4_acc, FIB4_AUROC, FIB4_AUPRC, FIB4_dets]))
    ...
    ENS3_low_preds, ENS3_low_probs, ENS3_low_test, ENS3_low_indet_count, ENS3_low_det_indices = ENS_class(ML_probs[:,5], APRI_probs, FIB4_probs,  X_test_imp_scl, Y_test, ENS3_params)
    ENS3_low_sens, ENS3_low_spec, ENS3_low_ppv, ENS3_low_npv, ENS3_low_acc = my_confusion_matrix(ENS3_low_test,ENS3_low_preds)

    try:
        ENS3_low_precs_py, ENS3_low_recs_py, thresholds = precision_recall_curve(ENS3_low_test/4, ENS3_low_probs)
        ENS3_low_AUPRC = auc(ENS3_low_recs_py, ENS3_low_precs_py)
    except ValueError:
        ENS3_low_AUPRC = np.nan

    try: 
        ENS3_low_fprs_py, ENS3_low_tprs_py, threshold = roc_curve(ENS3_low_test/4, ENS3_low_probs)
        ENS3_low_AUROC = roc_auc_score(np.asarray(ENS3_low_test)/4, ENS3_low_probs)
    except ValueError:
        ENS3_low_AURPC = np.nan
    
    ENS3_low_dets = 1-ENS3_low_indet_count/n_size


    ################################### ENS3_HIGH
    ENS3_params['threshold'] = 0.665
    ENS3_high_preds, ENS3_high_probs, ENS3_high_test, ENS3_high_indet_count, ENS3_high_det_indices = ENS_class(ML_probs[:,5], APRI_probs, FIB4_probs,  X_test_imp_scl, Y_test, ENS3_params)
    ENS3_high_sens, ENS3_high_spec, ENS3_high_ppv, ENS3_high_npv, ENS3_high_acc = my_confusion_matrix(ENS3_high_test,ENS3_high_preds)

    try:
        ENS3_high_precs_py, ENS3_high_recs_py, thresholds = precision_recall_curve(ENS3_high_test/4, ENS3_high_probs)
        ENS3_high_AUPRC = auc(ENS3_high_recs_py, ENS3_high_precs_py)
    except ValueError:
        ENS3_high_AUPRC = np.nan

    try: 
        ENS3_high_fprs_py, ENS3_high_tprs_py, threshold = roc_curve(ENS3_high_test/4, ENS3_high_probs)
        ENS3_high_AUROC = roc_auc_score(np.asarray(ENS3_high_test)/4, ENS3_high_probs)
    except ValueError:
        ENS3_high_AURPC = np.nan
    
    ENS3_high_dets = 1-ENS3_high_indet_count/n_size


    ENS3_low_results[:,i] = 100*np.transpose(np.array([ENS3_low_sens, ENS3_low_spec, ENS3_low_ppv, ENS3_low_npv, ENS3_low_acc, ENS3_low_AUROC, ENS3_low_AUPRC, ENS3_low_dets]))   
    ENS3_high_results[:,i] = 100*np.transpose(np.array([ENS3_high_sens, ENS3_high_spec, ENS3_high_ppv, ENS3_high_npv, ENS3_high_acc, ENS3_high_AUROC, ENS3_high_AUPRC, ENS3_high_dets]))     
    print('Completed iteration # ' + str(i))

ENS3_df_low = pd.DataFrame.from_records(ENS3_low_results)
ENS3_df_low = ENS3_df_low.rename(index={0: 'sens',1: 'spec',2: 'ppv',3: 'npv',4: 'acc',5: 'AUROC',6: 'AUPRC',7: 'det'})
ENS3_df_low.to_excel(path + ds + '_ENS3_bootstrap_0.45_paired.xlsx')

ENS3_df_high = pd.DataFrame.from_records(ENS3_high_results)
ENS3_df_high = ENS3_df_high.rename(index={0: 'sens',1: 'spec',2: 'ppv',3: 'npv',4: 'acc',5: 'AUROC',6: 'AUPRC',7: 'det'})
ENS3_df_high.to_excel(path + ds + '_ENS3_bootstrap_0.665_paired.xlsx')
